import logging
import uuid
import hashlib
import json
from datetime import datetime, timezone
from typing import Dict, Any

from shared_libs.core.contracts.execution import PluginResult, Evidence

logger = logging.getLogger("ValidationWorker")

class ValidationWorker:
    def execute(self, payload: Dict[str, Any]) -> PluginResult:
        logger.info("ValidationWorker executing")
        start_time = datetime.now(timezone.utc)

        execution_id = payload.get("execution_id", str(uuid.uuid4()))
        graph_id = payload.get("graph_id", "unknown")
        capability = payload.get("capability", "validation")
        parameters = payload.get("parameters", {})

        data = parameters.get("data", [])
        required_cols = parameters.get("required_columns", [])

        results = {}
        if data and isinstance(data, list) and isinstance(data[0], dict):
            missing_cols = [col for col in required_cols if col not in data[0]]
            results["schema_valid"] = len(missing_cols) == 0
            results["missing_columns"] = missing_cols
            results["record_count"] = len(data)
        else:
            results["error"] = "Invalid payload format. Expected list of dicts."

        end_time = datetime.now(timezone.utc)
        duration_ms = (end_time - start_time).total_seconds() * 1000

        output_str = json.dumps(results, sort_keys=True)
        computed_hash = hashlib.sha256(output_str.encode('utf-8')).hexdigest()
        confidence = 1.0 if "error" not in results else 0.0

        evidence = Evidence(
            evidence_id=str(uuid.uuid4()),
            execution_id=execution_id,
            graph_id=graph_id,
            worker="validation_worker",
            worker_version="1.0",
            tool="python",
            tool_version="3.11",
            capability=capability,
            source="ValidationWorker",
            timestamp=end_time.isoformat(),
            confidence=confidence,
            hash=computed_hash,
            checksum=computed_hash[:16],
            metadata={"schema_checked": True},
            version="1.0"
        )

        return PluginResult(
            version="1.0",
            plugin="validation_worker",
            worker="validation_worker",
            worker_version="1.0",
            capability=capability,
            status="SUCCESS" if "error" not in results else "FAILED",
            started_at=start_time.isoformat(),
            finished_at=end_time.isoformat(),
            duration_ms=duration_ms,
            confidence=confidence,
            metadata=results,
            evidence=[evidence]
        )