...
class StatisticsWorker:
    def execute(self, payload: Dict[str, Any]) -> PluginResult:
        ...
        if len(data) > 1:
            ...
            results["error"] = "Insufficient data for statistics"
        ...
        confidence = min(1.0, len(data) / 30.0) if len(data) > 1 else 0.0

        evidence = Evidence(
            evidence_id=str(uuid.uuid4()),
            execution_id=execution_id,
            graph_id=graph_id,
            worker="statistics_worker",
            worker_version="1.0",
            tool="python",
            tool_version="3.11",
            capability=capability,
            source="StatisticsWorker",
